...
/* 현재 스레드를 대기 상태로 전환합니다. thread_unblock()에 의해 깨어날 때까지 다시 스케줄되지 않습니다.
   이 함수는 인터럽트가 꺼진 상태에서 호출되어야 합니다.
   synch.h에 있는 동기화 프리미티브 중 하나를 사용하는 것이 보통 더 좋은 생각입니다. */
...
/* CPU를 양보합니다. 현재 스레드는 대기 상태로 설정되지 않으며,
 스케줄러의 재량에 따라 즉시 다시 스케줄될 수 있습니다. */
void thread_yield(void)
{
	struct thread *curr = thread_current(); // 현재 스레드 포인팅
	enum intr_level old_level;				// 인터럽트 레벨 구조체 변수 선언

	ASSERT(!intr_context());

	old_level = intr_disable();					  // 인터럽트 비활성화 정의, do_schedule 하려면 인터럽트가 비활성화 되어야함.
	if (curr != idle_thread)					  // 현재 스레드가 idle 상태가 아니면
		list_push_back(&ready_list, &curr->elem); // ready_list에 현재 스레드를 push
	do_schedule(THREAD_READY);					  // 새 프로세스를 스케쥴링
	intr_set_level(old_level);					  // 인터럽트 비활성화
}
...
void thread_awake(int64_t ticks)
{
    ...
    while (e != list_end(&sleep_list))
    {
        ...
        // 현재 스레드의 깨어날 시간이 주어진 ticks보다 작거나 같은 경우
        if (t->wakeup_tick <= ticks)
        {
            e = list_remove(e); // 슬립 큐에서 해당 스레드 제거
            thread_unblock(t);  // 스레드를 깨워서 준비 상태로 변경
        }
        else
        {
            e = list_next(e);
